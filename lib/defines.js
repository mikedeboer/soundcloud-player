...
const check = function(expr, defines) {
  try {
...
// Derived from the list at https://developer.mozilla.org/en-US/docs/OS_TARGET
const platformMap = {
  "linux": ["android", "linux"],
  "osx": ["darwin"],
  "unix": ["darwin", "sunos", "freebsd", "openbsd", "netbsd", "irix64", "aix",
           "hp-ux", "dragonfly", "osf1"],
  "win": ["winnt"]
};

const setPlatformFlags = function(defines = {}) {
  let platform = OS.toLowerCase();

  if (platformMap.linux.indexOf(platform) > -1)
    defines.XP_LINUX = 1;
  if (platformMap.osx.indexOf(platform) > -1)
    defines.XP_OSX = 1;
  if (platformMap.unix.indexOf(platform) > -1)
    defines.XP_UNIX = 1;
  if (platformMap.win.indexOf(platform) > -1)
    defines.XP_WIN = 1;
...
exports.parse = function(content, defines, path, devMode) {
  defines = setPlatformFlags(defines);
